...
const areaProductFilterGroup = document.getElementById('area-product-filter-group');
const areaProductCheckboxes = document.getElementById('area-product-checkboxes');
...
function setupEventListeners() {
    ...
    // Area Activity Change -> Populate Product Filter
    areaActivitySelect.addEventListener('change', () => {
        const activityName = areaActivitySelect.value;
        areaProductCheckboxes.innerHTML = '';
        areaResultContainer.style.display = 'none';
        
        if (!activityName) {
            areaProductFilterGroup.style.display = 'none';
            return;
        }
        
        const activityProducts = state.products.filter(p => (p.activity_name || 'Kegiatan Umum') === activityName);
        
        if (activityProducts.length <= 1) {
            // No need for a filter if there's only 1 product
            areaProductFilterGroup.style.display = 'none';
            return;
        }
        ...
        activityProducts.forEach(p => {
            ...
            label.style.cssText = 'display: flex; align-items: center; gap: 8px; cursor: pointer; padding: 6px 4px; border-radius: 6px; transition: background 0.2s;';
            label.innerHTML = `
                <input type="checkbox" class="area-product-cb" value="${p.id}" style="accent-color: var(--primary); width: 18px; height: 18px;">
                <span style="font-size: 0.95rem;">${p.product_name}</span>
                <span style="font-size: 0.8rem; color: var(--text-muted); margin-left: auto;">${p.dose_per_ha} ${p.dose_unit || 'L'}/Ha</span>
            `;
            label.addEventListener('mouseenter', () => label.style.background = 'rgba(255,255,255,0.05)');
            label.addEventListener('mouseleave', () => label.style.background = 'transparent');
            areaProductCheckboxes.appendChild(label);
        });
        
        areaProductFilterGroup.style.display = 'block';
    });
    ...
    btnCalcArea.addEventListener('click', () => {
        ...
        const allActivityProducts = state.products.filter(p => (p.activity_name || 'Kegiatan Umum') === activityName);
        
        // Get checked product IDs
        const checkedBoxes = document.querySelectorAll('.area-product-cb:checked');
        const selectedIds = Array.from(checkedBoxes).map(cb => cb.value);
        
        // If none checked, use all products. If some checked, filter.
        const activityProducts = selectedIds.length > 0 
            ? allActivityProducts.filter(p => selectedIds.includes(p.id))
            : allActivityProducts;

        if (activityProducts.length > 0) {
            // Use the first product's water rate (uniform for the activity)
            const waterRate = allActivityProducts[0].water_rate_per_ha;
